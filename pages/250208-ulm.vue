<template>
  <section
    class="hero is-medium is-info"
    style="
      background-image: url(/250208-ulm.jpg);
      background-size: cover;
      background-position: 50% 50%;
    "
  >
    <div class="hero-body">
      <h1 class="title has-text-black">Singlebörse Schachmatt Ulm</h1>
      <p class="subtitle has-text-dark">
        Samstag, 08.02.2025 @ Maritim Hotel Ulm
      </p>
    </div>
  </section>

  <div class="container is-max-desktop">
    <p class="block mt-4">
      Alleine unterwegs und auf der Suche nach jemand der mit dir läuft? Dann
      bist du hier genau richtig! Schau wer bereits nach einem Laufpartner sucht
      und nimm Kontakt auf:
    </p>

    <iframe
      src="https://airtable.com/embed/appPvmwKKIiUVz7Z5/shr2nc78Ex9kFP8pj?layout=card"
      frameborder="0"
      width="100%"
      height="533"
      style="background: transparent"
      class="block"
    ></iframe>

    <p class="block">
      Nichts passendes gefunden? Dann leg einfach einen neuen Eintrag an und wir
      drücken die Daumen, dass sich jemand bei dir meldet! Trotzdem ist
      regelmäßig wieder reinschauen sicher kein Fehler ;)
    </p>

    <iframe
      class="airtable-embed block"
      src="https://airtable.com/embed/appPvmwKKIiUVz7Z5/shrjjek1LsDIsINMi"
      frameborder="0"
      width="100%"
      height="533"
      style="background: transparent"
    ></iframe>
  </div>
</template>

<script setup>
useSeoMeta({
  title: "08.02.25 Ulm",
  ogImage: "/250208-ulm-og.jpg",
});
</script>
